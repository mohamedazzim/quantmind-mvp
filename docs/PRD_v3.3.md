# QuantMind — Product Requirements Document v3.3
...
The production harness does not accept arbitrary `signal_fn` callables or `signal_column` inputs. Synthetic regression fixtures may use those inputs only through the explicitly named test-fixture path, which is not part of the agent research API.
