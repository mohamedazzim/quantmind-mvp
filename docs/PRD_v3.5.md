# QuantMind — Product Requirements Document v3.5
...
Convert a hypothesis into a declarative, machine-readable strategy specification.
...
The production builder must emit only declarative fields. The runtime compiles `signal_name` plus its parameters through a fixed whitelist of registered implementations. Arbitrary agent-generated Python, executable code strings, and precomputed signal columns are not permitted in production research trials.

`strategy_id` is derived from the canonical strategy specification rather than caller-supplied labels.
...
The strong control is not defined only by a qualitative label. It is a versioned injected edge measured in net basis points per triggered trade. RP-2 deliberately uses a sparse, observable condition so the control does not create an obvious dense momentum market.
...
# 46A. Dataset Registry & Production Data Boundary

Production research must never accept a caller-supplied historical DataFrame.

```text
Agent Research Request
        ↓
dataset_version + split_zone
        ↓
Dataset Registry
        ↓
...
The registry supports exactly `LICENSED` and `SYNTHETIC`. The production harness accepts only `LICENSED`. Synthetic datasets are reserved for the testing harness.
...
A versioned dataset may expose `RESEARCH`, `VALIDATION`, `HOLDOUT`, and `FORWARD` zones. The loader verifies the file checksum before returning only the requested named zone.

## Production boundary

`ResearchHarness.run_trial()` accepts a dataset version and named split zone. It does not accept a DataFrame, `signal_fn`, or `signal_column`. Synthetic regression fixtures use `quantmind.testing.FixtureResearchHarness`; fixture trials record `mode=FIXTURE` and `dataset_kind=SYNTHETIC` and do not contribute to production budgets, multiplicity populations, or approval gates.
...
# 59A. StrategySpec Validation & Variant Identity

StrategySpec validation and normalization occur before trial reservation.

For the MVP whitelist:

```text
calendar_session_bars → exact int, > 1
session_window → exactly two finite numeric values
0 <= start < end <= 1
```

Strings, booleans, invalid ranges, missing parameters, unknown parameters, and non-finite values are rejected at compile time. An invalid StrategySpec creates no trial row and consumes no budget.

## Variant identity

Variant identity is the SHA-256 of normalized logic only:
...
`strategy_version` is metadata and does not mint a new variant when logic is unchanged.

# 60. Trial-Ledger Security Test

The trial count cannot be incremented manually by agents.

The **ResearchHarness is the sole supported research-trial entry point**. Agents receive research tools that call the harness; they do not receive direct access to `BacktestEngine.run`, trial reservation, trial-ID creation, or budget accounting.

The deterministic engine entry point is internal to the harness. Direct calls to the public engine API are rejected.

The harness owns:

- Mandatory causality preflight
- StrategySpec compilation through the registered whitelist
- Trial-ID generation
- Experiment-ID derivation from research task
- Strategy-ID derivation from the complete canonical StrategySpec
- Cumulative budget reservation
- Failure recording
- Completion recording

The production harness does not accept arbitrary `signal_fn` callables, `signal_column` inputs, or caller-supplied DataFrames. Synthetic regression fixtures use those inputs only through `quantmind.testing.FixtureResearchHarness`, which is outside the production research API.

The system must reject attempts to:

- Modify previous trial counts
- Delete trial records
- Rewrite terminal trial results
- Mark a terminal trial as skipped
- Reset cumulative budgets by starting another research task
- Bypass the harness to run an uncounted research trial

SQLite MVP protections include an append-only DELETE trigger and a terminal-row UPDATE trigger. The production PostgreSQL design will enforce the same invariant with database roles so research agents cannot mutate historical trial records.

# 60A. Trial Lifecycle

Every harness-started trial follows:
...
RESERVED / RUNNING
   ...
   ┌────┴────────────────────┐
   ↓                         ↓
COMPLETED            FAILED / REJECTED_NONCAUSAL
   │
   └────────────── terminal

RUNNING → ABANDONED
        via stale-run reaper
```

Terminal rows are immutable and remain part of cumulative trial, experiment, strategy-variant, runtime, and multiplicity accounting.

The budget reservation is atomic across processes and includes in-flight reservations through their estimated runtime/cost. Stale runs are marked `ABANDONED`, never deleted.

Every trial also records `mode=PRODUCTION|FIXTURE` and `dataset_kind=LICENSED|SYNTHETIC`. Production population queries use `mode=PRODUCTION`, excluding fixture rows by construction.
...
- Production trials load data through the checksum-verified Dataset Registry and named split zone.
- Production trials reject `SYNTHETIC` datasets.
- Invalid StrategySpecs fail before reservation.
- Fixture trials are excluded from production populations.
...
# 69A. Backtester Baseline

The deterministic futures backtester is the next implementation stage. Its first regression suite must prove:

```text
1. Look-ahead canary:
   next-bar-open P&L ≈ 0 incremental capture
   same-close canary P&L ≈ +30 bps relative to base

2. Positive control:
   next-bar-open net P&L tracks injected net edge at each ladder rung

3. Directional null:
   prior-bar momentum has near-zero gross edge and remains net negative after costs
```

The control ladder is not a statistical detection threshold by itself. Its trigger frequency and lag-1 autocorrelation are fixture-quality constraints; actual detectability is evaluated later through the full research-integrity pipeline.
